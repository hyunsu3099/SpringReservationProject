 ...
 * 작성일 : 2022-11-28, 최종수정 2022-12-16
 */
...
public class ReservationServiceImp implements ReservationService {
    ...
    public List<ReservationStatusDto> getReservatrionStatuses(String startdate, String enddate, int reservationId) throws ParseException{
        ...
        for(OrderView order : orders){
            ...
            for(ReservationTime time : times){
                String timeStr = time.getStartTime();
        ...
        // 2-2. 각 날짜+시간에 주문이 가능한지를 set을 통해 확인하고 주문가능여부 status를 입력한다.
        ...
        while(cal.before(end)){
            ...
            // 2-1. 시작날짜부터 순서대로 각 시간객체를 생성 (복사)
            for(ReservationTime time : times){
                ReservationStatusDto dto = time.getReservationStatusDto();
                dto.setReservationDate(dateStr);

                // 2-2. 각 날짜+시간에 주문이 가능한지를 set을 통해 확인하고 주문가능여부 status를 입력한다.
                if(set.contains(dateStr+time.getStartTime())){
                    dto.setStatus(0);
                }
            }
    ...
    public List<ReservationStatusDto> getReservatrionStatuses(){
        ...
        return getReservatrionStatuses(startdate, enddate);
    }
